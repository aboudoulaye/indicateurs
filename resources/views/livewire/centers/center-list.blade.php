    ...
    <div class="bg-base-100 rounded-xl shadow-md p-4 mt-4 w-full">
        ...
        <div class="flex flex-wrap justify-between items-center mb-4 gap-2">
            <span class="badge badge-warning font-semibold p-3">
                LISTE DES ÉTABLISSEMENTS ({{ $nbrecentres }})
            </span>
                ...
                <table class="table table-zebra w-full">
                    ...
                    <tbody>
                        @foreach ($centres as $center)
                            <tr wire:key="{{ $center->id }}" class="hover:bg-base-200">
                                <td>{{ $center->id }}</td>
                                <td>{{ $center->code }}</td>
                                <td>
                                    @if ($editCenterID === $center->id)
                                        <div class="flex flex-col">
                                            <input wire:model="editCenterName" type="text"
                                ...
                                <td class="text-right">
                                    <div class="flex justify-end gap-2">
                                        ...
                                        @else
                                            <button wire:click="edit({{ $center->id }})"
                                            ...
                                            <button
                                                onclick="if(confirm('Êtes-vous sûr ?')) { @this.delete({{ $center->id }}) }"
                                                class="btn btn-ghost btn-square">
                                                <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24"
