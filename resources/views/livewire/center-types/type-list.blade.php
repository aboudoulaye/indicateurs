    ...
    <div class="bg-base-100 rounded-xl shadow-md p-4 mt-4 w-full">
        ...
        <div class="flex flex-wrap justify-between items-center mb-4 gap-2">
            <span class="badge badge-warning font-semibold p-3">
                LISTE DES TYPES ÉTABLISSEMENTS  ({{$nbretypes}})
            </span>
        ...
        <div class="card-body p-0">
            <div class="overflow-x-auto">
                <table class="table table-zebra w-full">
                    <thead>
                        <tr>
                            <th class="w-1/6 min-w-[80px]">Id</th>
                            <th class="w-3/5 min-w-[200px] text-center">Nom</th>
                            <th class="w-1/4 min-w-[140px] text-right">Actions</th>
                        </tr>
                    </thead>
                    <tbody>
                        @foreach ($types as $type)
                            <tr wire:key="{{ $type->id }}" class="hover:bg-base-200">
                                <td>{{ $type->id }}</td>
                                <td class="text-center">
                                    @if ($editTypeID === $type->id)
                                        <div class="flex flex-col items-center">
                                            <input wire:model="editTypeName" type="text"
                                                class="input input-bordered w-full p-2 text-center" pattern="^[A-Z\-_]+$"
                                ...
                                <td class="text-right">
                                    <div class="flex justify-end gap-2">
                                        ...
                                        @else
                                            ...
                                            <button
                                                onclick="if(confirm('Êtes-vous sûr ?')) { @this.delete({{ $type->id }}) }"
                                                class="btn btn-ghost btn-square">
                                                <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24"
