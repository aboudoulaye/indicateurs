    ...
    <div class="bg-base-100 rounded-xl shadow-md p-4 mt-4 w-full">
        ...
        <div class="flex flex-wrap justify-between items-center mb-4 gap-2">
            <span class="badge badge-warning font-semibold p-3">
                LISTE DES PRODUITS/PROGRAMMES ({{ $nbr }})
            </span>
            ...
            <div class="overflow-x-auto">
                <table class="table table-zebra w-full">
                    <thead>
                        <tr>
                            <th class="w-[5%]">N°</th>
                            <th class="w-[30%]">Pragrammes</th>
                            <th class="w-[20%]">Codes Produits</th>
                            <th class="w-[300%]">Produits</th>
                            <th class="w-[20%] text-right">Actions</th>
                        </tr>
                    </thead>
                    <tbody>
                        @foreach ($productPrograms as $prodsprogr)
                            <tr wire:key="{{ $prodsprogr->product->id }}-{{ $prodsprogr->program->id }}"
                                class="hover:bg-base-200">
                                <td>{{ $i++ }}</td>
                                <td>{{ $prodsprogr->program->name }}</td>
                                <td>{{ $prodsprogr->product->code }}</td>
                                <td>
                                    {{ $prodsprogr->product->name }}
                                </td>

                                <!-- Colonne Actions déplacée en dernière position -->
                                <td class="text-right">
                                    <div class="flex justify-end gap-2">
                                        <button onclick="return confirm('Êtes-vous sûr de rompre la liaison ?')"
                                            wire:click="delete({{ $prodsprogr->product->id }}, {{ $prodsprogr->program->id }})"
